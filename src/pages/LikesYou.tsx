...
const LikesYou = () => {
  ...
  useEffect(() => {
    const fetchLikers = async () => {
      ...
      const { data: profiles } = await supabase
        .from("profiles")
        .select("id, name, age, profile_image, location, bio")
        .in("id", likerIds);
  ...
  const handleConnect = async (likerId: string) => {
    ...
    // Create a right swipe back → triggers match
        ...
        <div className="container max-w-md mx-auto px-4 py-6">
          ...
          <div className="text-center py-16 text-muted-foreground">
            ...
            <>
              {!isPremium && (
                <Card className="mb-6 border-primary/30 bg-gradient-to-br from-primary/5 to-primary/10">
                  <CardContent className="p-4 text-center">
                    ...
                    <p className="text-sm text-muted-foreground mb-3">
                      Upgrade to Premium to see who they are and connect
                    </p>
                    <Button onClick={() => navigate("/premium")} className="w-full">
                      <Lock className="h-4 w-4 mr-2" />
              ...
              <div className="grid grid-cols-2 gap-3">
                {likers.map(liker => (
                  <Card key={liker.id} className="overflow-hidden relative">
                    <div className="relative h-44 bg-gradient-to-br from-primary/20 to-secondary/20">
                      {liker.profile_image ? (
                        <img
                          ...
                          className={`absolute inset-0 w-full h-full object-cover ${!isPremium ? "blur-xl" : ""}`}
                        />
                      ) : (
                        <div className="absolute inset-0 flex items-center justify-center">
                          <Heart className={`h-10 w-10 text-primary/30 ${!isPremium ? "blur-lg" : ""}`} />
                        </div>
                      )}
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-2">
                        <p className={`text-white font-semibold text-sm ${!isPremium ? "blur-md" : ""}`}>
                          {liker.name}, {liker.age}
                        </p>
                        {liker.location && (
                          <p className={`text-white/80 text-xs ${!isPremium ? "blur-md" : ""}`}>
                            {liker.location}
                          </p>
                        )}
                      </div>
                    ...
                    {isPremium && (
                      <div className="flex">
                        <Button
                          variant="ghost"
                          size="sm"
                          className="flex-1 rounded-none text-destructive hover:bg-destructive/10"
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="flex-1 rounded-none text-primary hover:bg-primary/10"
                          onClick={() => handleConnect(liker.id)}
                        >
