...
const Chat = () => {
  ...
  const loadChatData = async () => {
    ...
    if (existingMessages) {
      setMessages(existingMessages.map(msg => ({
        ...msg,
        read: msg.sender_id === session.user.id || Math.random() > 0.3,
        delivered: true,
      })));
    }
      ...
      .on(
        ...
        (payload) => {
          ...
          setMessages(prev => [...prev, {
            ...newMsg,
            delivered: true,
            read: newMsg.sender_id === session.user.id,
          }]);
        }
      )
    ...
    if (error) {
      ...
      setTimeout(() => {
        setMessages(prev => prev.map(msg => 
          msg.sender_id === currentUser.id && !msg.read
            ? { ...msg, read: true }
            : msg
        ));
  ...
  const handleTyping = (value: string) => {
    ...
    channelRef.current.send({
      type: 'broadcast',
      event: 'typing',
      payload: { userId: currentUser.id, isTyping: value.length > 0 },
    });
