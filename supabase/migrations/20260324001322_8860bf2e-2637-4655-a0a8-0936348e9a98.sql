CREATE OR REPLACE FUNCTION public.get_likers_for_user(_user_id uuid)
 RETURNS TABLE(id uuid, name text, age integer, profile_image text, location text, bio text, is_super_like boolean, is_premium boolean, liker_count integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Count likers
  SELECT count(*)::integer INTO _liker_count
  ...
  ORDER BY EXISTS (SELECT 1 FROM public.super_likes sl WHERE sl.sender_id = sw.swiper_id AND sl.receiver_id = _user_id) DESC;
END;
$$;